...
* 一方で、ほとんどのビジネスロジックがサービスレイヤの中のトランザクションスクリプトにある場合がある。この場合ドメインオブジェクトは非常にシンプルである
    * もしドメインモデルが DB と one-toone であれば `Active Record`
        のようなシンプルなデータソースレイヤを使うことができる
* controller-entity というスタイルもある
    * ポイントはロジックを1つのトランザクションに持つか、トランザクションスクリプトにあるユースケースに持つこと。これらはコントローラやサービスと呼ばれる
